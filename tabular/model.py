...
class Model(object):
  ...
  def __init__(self, metadata):
    ...
    self.num_epochs_we_want_to_train = 25
  ...
  def model_fn(self, features, labels, mode):
    ...
    x_skip = self.fc(x, hidden_features, is_training)
    x = self.fc(x_skip, hidden_features, is_training)
    x = tf.nn.dropout(x, keep_prob)
    x = self.fc(x, hidden_features, is_training) + x_skip
    x_mid = self.fc(x, f, is_training)

    x = self.fc(x, hidden_features, is_training)
    for i in range(3):
        x = self.fc(x, hidden_features, is_training)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      optimizer = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.999, beta2=0.999, epsilon=1e-7)
